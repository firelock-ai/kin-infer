...
impl GpuKernelPlan {
    ...
    pub fn resolve(profile: Profile, backend: AcceleratorBackend) -> GpuKernelPlan {
        ...
        if matches!(profile, Profile::Throughput) && matches!(backend, AcceleratorBackend::Metal) {
            ...
            // numerically identical to the 32x32 MMA by construction (same fp32
            // `simdgroup_float8x8` accumulate, same per-fragment reduction order —
            // only the output blocking changes), but the wider register tile raises
            // per-thread register pressure enough to cut GPU occupancy on the embed
            // forward: it regresses real embed throughput several-fold on Metal
            // rather than helping. It remains opt-in per lever via `KIN_INFER_MMA_WIDE`
            // so the kernel can still be A/B-measured in isolation.
            //
            // `flash_attention` also stays off. The fused online-softmax kernel
            // changes the attention reduction order — its matches-CPU test is
            // tolerance-based (5e-4), not bit-identical — unlike steel which is
            // identical by construction, so it can perturb embedding values. It
            // remains opt-in via `KIN_INFER_FLASH_ATTENTION=1`.
        }
        plan
    }
