...
- `src/lib.rs` is the core engine: model loading, forward pass, SIMD primitives (~2100 lines)
- `src/gpu.rs` is the GPU abstraction: `GpuCompute` trait, device discovery, CPU fallback
- `src/metal_backend.rs` is the Apple Metal backend: custom MSL compute shaders for matmul, softmax, norms, activations
- `src/cuda_backend.rs` is the NVIDIA CUDA backend: PTX kernels via driver API FFI (no toolkit needed at build time)
