...
fn cos(a: &[f32], b: &[f32]) -> f64 {
    let mut d = 0.0f64;
    for i in 0..a.len() { d += a[i] as f64 * b[i] as f64; }
    d
}
...
fn forward_determinism() {
    ...
    let ref0 = model.forward(&[ids.clone()], &[m.clone()]).unwrap().pop().unwrap();
    let mut worst = 1.0f64;
    let mut worst_k = 0usize;
    const REPEATS: usize = 40;
    for k in 1..=REPEATS {
        let e = model.forward(&[ids.clone()], &[m.clone()]).unwrap().pop().unwrap();
        let c = cos(&ref0, &e);
        if c < worst { worst = c; worst_k = k; }
    }
    let verdict = if worst < 0.99 { "NONDETERMINISTIC" } else { "deterministic" };
